...
int main(int argc, char *argv[])
{
    ...
    while (remaining_files > 0)
    {
        ...
        for (int i = 0; i < SLAVE_QTY; i++)
        {
            if (!is_closed(slavesInfo[i].pipes[SLAVE_TO_APP].fdR))
        ...
        for (int i = 0; i < SLAVE_QTY; i++)
        {
            ...
            if (FD_ISSET(fdSlave, &fdSet))
            {
                ...
                { // lee los datos mas un \n
                    ...
                    char** tokens = split_string(buffer, DELIMITER, &num_tokens);
                    if(num_tokens != 3){
                        ERROR_HANDLING(TOKENS_ERROR_QTY);
                    }*/

                    FILE *file = fopen("results.txt", "a");

                    if (file == NULL)
                        ERROR_HANDLING(FILE_OPENING);

                    // fprintf(file, "PID: %s FILE: %s, MD5: %s\n", tokens[0], tokens[1], tokens[2]);
                    ...
                    /*shm_main_ptr[shm_index].slaveID = atoi(tokens[0]);
                    strncpy(shm_main_ptr[shm_index].response, tokens[2], MD5_SIZE);
                    strcpy(shm_main_ptr[shm_index].fileName, tokens[1]); // el nombre de los archivos no puede ser mayor a MAX_FILENAME

                    shm_main_ptr[shm_index].response[MD5_SIZE-1] = '\0';
                    */
                ...
                { // el slave ya no tiene archivos a procesar
                    ...
                    { // no hay mas files para procesar
                        close(slavesInfo[i].pipes[APP_TO_SLAVE].fdW);
                        close(slavesInfo[i].pipes[APP_TO_SLAVE].fdR);
                        close(slavesInfo[i].pipes[SLAVE_TO_APP].fdR);
...
void setup_slaves(TSlaveInfo *slavesInfo, int slotSize)
{
    for (int i = 0; i < SLAVE_QTY; i++)
    {
        int pipe1[2];
        if (pipe(pipe1) == -1)
            ERROR_HANDLING(APP_SLAVE_PIPE_CREATION);
        ...
        int pipe2[2];
        if (pipe(pipe2) == -1)
            ERROR_HANDLING(SLAVE_APP_PIPE_CREATION);
